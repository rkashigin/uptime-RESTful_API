        ...
        _data.read('users', phone, function (err, userData) {
            if (!err && userData) {
                ...
                if (hashedPassword === userData.hashedPassword) {
                    ...
                    callback(400, {
                        'Error': 'Password did not match the cpsacified user\'s stored password'
                    });
                }
            } else {
                callback(400, {
...
// Verifying if a given token id is currently valid for a given user
...
// Containter for all the checks methods
handlers._checks = {};
            ...
            if (!err && tokenData) {
                ...
                _data.read('users', userPhone, function (err, userData) {
                    if (!err && userData) {
                        ...
                        // Varify that the user has less than the number of max-checks-per-user
                        if (userChecks.length < config.maxChecks) {
                            // Create a random id for the check
                            var checkId = utils.createRandomString(20);
...
handlers._checks.put = function (data, callback) {
    // Check for the required field to be valid
    var id = typeof (data.payload.id) === 'string' && data.payload.id.trim().length === 20 ? data.payload.id.trim() : false;
                                ...
                                _data.read('users', checkData.userPhone, function (err, userData) {
                                    if (!err && userData) {
                                        ...
                                        if (checkPosition > -1) {
                                            ...
                                            callback(500, {
                                                'Error': 'Could not find the check in the user object so could not remove it'
                                            });
                                        }
                                    } else {
                                        callback(500, {
                                            'Error': 'Could not find the user who created the check, so it\'s impossible to remove the check from the list of checks in the user object'
                                        });
                                    }
                                });
                            } else {
